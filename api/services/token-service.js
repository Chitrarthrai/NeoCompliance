...
class TokenService {
  ...
  storeRefreshToken = async (userId, token) => {
    ...
    if (!isExist) return await TokenModel.create({ userId, tokens });
    else
  ...
  verifyAccessToken = (accessToken) =>
    jwt.verify(accessToken, accessTokenSecretKey);

  findRefreshToken = async (userId, token) =>
    await TokenModel.findOne({ userId, "tokens.token": token }).select({
